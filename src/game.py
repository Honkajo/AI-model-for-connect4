...
class Connect4:
    ...
    def start_game(self):
        ...
        while True:
            ...
            if current_turn == "ai":
                chosen_col = self.iterative_deepening(5, last_move)
    ...
    def iterative_deepening(self, time_limit, move):
        ...
        while True:
            if time.time() - start_time >= time_limit:
                break
            current_col, current_eval = self.minmax(
                depth, float('-inf'), float('inf'), True, move)
    ...
    def minmax(self, depth, alpha, beta, maximizingPlayer, move):
        ...
        valid_moves = self.sort_moves(
            [col for col in self.preferred_cols if self.is_valid_move(col)])
        ...
        if maximizingPlayer:
            ...
            for col in valid_moves:
                ...
                move = (row, col)
                _, eval = self.minmax(depth-1, alpha, beta, False, move)
            ...
            for col in valid_moves:
                ...
                move = (row, col)
                _, eval = self.minmax(depth-1, alpha, beta, True, move)
                self.board[row][col] = 0
                if eval < min_eval:
                    min_eval = eval
                    best_col = col
            ...
            for col in range(7):
                ...
                if current_player != 0:
                    ...
                    if len(positive_slope) >= 2:
                        process_chain(positive_slope,
                                      current_player, handled_positive)
                    ...
                    if len(negative_slope) >= 2:
                        process_chain(negative_slope,
                                      current_player, handled_negative)
